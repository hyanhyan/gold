...
@section('content')
    ...
    <input type="hidden" class="delete-hidden" name="copyID" value="{{$copyID}}">
    <div class="row" id="myTabContent">
        ...
        <div class="img-cont col-md-6">
                        ...
                        @foreach ($product->pictures as $key => $picture)
                            ...
                            <div class="each">
                                ...
                                <div class="change-img">
                                    <img class="" src="{{ asset('/uploads/products/' . $picture->img_name )}}" alt="{{ $picture->img_name }}">
                                </div>
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                ...
                <input type="text" id="yt-input" class="form-control" value="{{$product->videoEmbed}}" placeholder="https://youtu.be/bWNmJqgri4Q">
                <input type="hidden" id="videoURL" class="form-control" value="{{$product->videoURL}}" name="prod[videoURL]">
                <iframe id="yt-iframe" width="260" height="115" src="{{$product->videoEmbed}}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
        </div>
    </div>
    ...
    <div class="row prod-edit">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
            <div class="form-group">
                ...
                {!! Form::select('prd[category]', [ 1 => Lang::get('lang.JEWELERY'), 2 => Lang::get('lang.TABLEWARE'), 3 => Lang::get('lang.BOX'), 4 => Lang::get('lang.WATCH') ], $product->product_global_type , array('class' => 'form-control','disabled' => true,  'id' => 'selectCategory')) !!}
            </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
            <div class="form-group" id="metalTypeDiv">
                ...
                <select class="form-control" name="prod[metal_id]" required id="metalType" {{$product->product_global_type == 2 ? 'disabled': ''}}>
                    <option value="">{{ Lang::get('lang.SELECT_CATEGORY')  }}</option>
                    @if ($product->product_global_type!= 2 && count($all_data['metals']))
                        @foreach($all_data['metals'] as $key=>$metal)
                            <option value="{{ $key }}" {{ $product->metal_id == $key ? 'selected=selected' : '' }}>{{ Lang::get('lang.'.$metal)  }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3" id="forWhom">
            <div class="form-group">
                ...
                {!! Form::select('prod[m_w_c]',[ '' => Lang::get('lang.FOR_WHOM'), 1 => Lang::get('lang.MEN') , 2 => Lang::get('lang.WOMEN') , 3 => Lang::get('lang.CHILDREN') ], $product->product_global_type != 2 ? $product->m_w_c : 0 , array('class' => 'form-control', $product->product_global_type == 2 ? 'disabled': '', 'required')) !!}
            </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
            <div class="form-group">
                ...
                <select class="form-control" name="prod[product_type_id]" >

                    @if (count($all_data['productType']))

                        @foreach($all_data['productType'] as $key=>$prType)
                            <option value="{{ $key }}" {{ $product->product_type_id == $key ? 'selected=selected' : '' }}>{{ Lang::get('lang.'.$prType)  }}</option>
                        @endforeach
                    @endif
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
            <div class="form-group" id="rateDiv">
                ...
                <select class="form-control" name="prod[rate_id]" required  id="metalRate" {{$product->product_global_type == 2 ? 'disabled': ''}}>
{{--                    <option value="0" >{{ Lang::get('lang.SELECT_FINENESS')  }}</option>--}}
                    @if ($product->product_global_type!= 2 && count($all_data['rates']))

                        <option value="" >{{ Lang::get('lang.SELECT_FINENESS')  }}</option>
                        @foreach($all_data['rates'] as $key=>$rate)
                            <option value="{{ $rate->id }}" {{ $product->rate_id == $rate->id ? 'selected=selected' : '' }}
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
            <div class="form-group">
                ...
                {!! Form::text('prod[code]', '', array('placeholder' => Lang::get('lang.PRODUCT_CODE'),'class' => 'form-control', 'required')) !!}
            </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
            <div class="form-group">
                ...
                {!! Form::select('prod[addresses][]', [ 'address' => $all_data['info']['address'], 'optional_address' => $all_data['info']['optional_address'] ], $product->addresses , array('class' => 'form-control', 'multiple' => 'multiple', 'required')) !!}
            </div>
        </div>
                    ...
                    @if (count($all_data['stones']))
                        ...
                        @foreach($all_data['stones'] as $key=>$stone)
                            <tr>
                                ...
                                <td><input class="form-control" name="stone[carat][]" min="0.1" step="0.1" type="number" value="{{ $stone['carat']}}"></td>
                                <td><input class="form-control" name="stone[pcs][]" min="1" step="1" type="number" value="{{ $stone['pcs']}}"></td>
                                <td>
            ...
            <div class="hidden-stone d-none">
                <table class="col-stone">
                    <tr>
                        ...
                        <td><input class="form-control" name="stone[pcs][]" value="1" min="1" step="1" type="number"></td>
                        <td>
                            {!! Form::select('stone[color][]', [ 'D' => 'D' , 'E' => 'E', 'F' => 'F' , 'G' => 'G',
                                                                'H' => 'H' , 'I' => 'I', 'J' => 'J' , 'K' => 'K' ],
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 text-center">
            <button type="submit" id="copy-btn" class="btn btn-primary">{!! Lang::get('lang.CREATE') !!}</button>
        </div>
    </div>
    {!! Form::close() !!}
@append
